...
namespace Database.Migrations
{
    ...
    public class PTokenMigration : IMigration
    {
        private readonly IMongoDatabase _database;
        private readonly IMongoCollection<PTokenDocument> _collection;

        public int Order => 5;
        public string Name => "Ptokens";
        public string Icon => "🔑";

        public PTokenMigration(IMongoDatabase database)
        {
            _database = database;
            _collection = database.GetCollection<PTokenDocument>("Ptokens");
        }
        ...
        private async Task CreateIndexes()
        {
            try
            {
                ...
                {
                    var tokenIndexKeysDefinition = Builders<PTokenDocument>.IndexKeys.Ascending(x => x.Token);
                    var tokenIndexOptions = new CreateIndexOptions
                    {
                        Unique = true,
                        Name = "TokenIndex"
                    };
                    var tokenIndexModel = new CreateIndexModel<PTokenDocument>(tokenIndexKeysDefinition, tokenIndexOptions);
                ...
                {
                    var userIdIndexKeysDefinition = Builders<PTokenDocument>.IndexKeys.Ascending(x => x.UserId);
                    var userIdIndexOptions = new CreateIndexOptions { Name = "UserIdIndex" };
                    var userIdIndexModel = new CreateIndexModel<PTokenDocument>(userIdIndexKeysDefinition, userIdIndexOptions);
                ...
                {
                    var expiresAtIndexKeysDefinition = Builders<PTokenDocument>.IndexKeys.Ascending(x => x.ExpiresAt);
                    var expiresAtIndexOptions = new CreateIndexOptions
                    {
                        Name = "ExpireAtIndex",
                        ExpireAfter = TimeSpan.FromHours(24)
                    };
                    var expiresAtIndexModel = new CreateIndexModel<PTokenDocument>(expiresAtIndexKeysDefinition, expiresAtIndexOptions);
